...
export const FormCierreDespacho = ({ despacho, onClose }) => {
  ...
  const onSubmit = async (data) => {
    const jsonData = { intento: data.intento, despachado: data.despachado };
    try {
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col justify-center text-center px-24 text-xl">
        ...
        <div className="mb-5">
          ...
          <input disabled type="text" value={despacho.idDespacho}
        ...
        <div className="mb-5">
          ...
          <input type="date" value={despacho.fechaDespacho} disabled
            className="border border-gray-300 rounded-lg block w-full text-slate-400 p-1" />
        </div>
        <div className="mb-5">
          <label className="block font-bold mb-2">Patente Camión</label>
          <input type="text" disabled value={despacho.patenteCamion}
            className="border border-gray-300 rounded-lg block w-full text-slate-400 p-1" />
        </div>
        <div className="mb-5">
          <label className="block font-bold mb-2">Intentos de entrega</label>
          <input type="number" defaultValue={despacho.intento}
            className="border border-gray-300 rounded-lg block w-full p-1"
            {...register("intento", { required: true })} />
        </div>
        <div className="mb-5">
          <label className="block font-bold mb-2">Despacho entregado</label>
          <select defaultValue={false} className="border border-gray-300 rounded-lg block w-full p-1"
        ...
        <div className="mb-5">
          ...
          <input type="text" disabled value={despacho.idCompra}
            className="border border-gray-300 rounded-lg block w-full text-slate-400 p-1" />
        </div>
        <div className="mb-5">
          <label className="block font-bold mb-2">Dirección Compra</label>
          <input type="text" disabled value={despacho.direccionCompra}
            className="border border-gray-300 rounded-lg block w-full text-slate-400 p-1" />
        </div>
        <div className="mb-5">
          <label className="block font-bold mb-2">Valor Compra</label>
          <input type="text" disabled value={despacho.valorCompra}
            className="border border-gray-300 rounded-lg block w-full text-slate-400 p-1" />
        </div>
